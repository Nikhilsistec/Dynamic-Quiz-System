{{-- Multiple choice: manage options via JSON API (edit) or inline fields (create) --}}
<p class="text-sm text-gray-500 dark:text-gray-400 mb-3">
    Add options below. Mark <strong>all correct</strong> answers.
    ...
    @foreach($question->options as $option)
        <div class="flex items-center gap-2" data-option-id="{{ $option->id }}">
            <input type="checkbox" value="{{ $option->id }}"
                   class="h-4 w-4 text-green-600 border-gray-300 dark:border-gray-600 correct-check"
                   {{ $option->is_correct ? 'checked' : '' }}
                   onchange="markCorrect({{ $option->id }}, '{{ route('admin.questions.options.update', [$question, $option]) }}', this.checked)">
            ...
            @if($option->image_path)
                <img src="{{ $option->image_url }}" class="h-8 w-8 object-cover rounded border" alt="">
            @endif
            <button type="button"
...
<div id="mc-inline-options" class="space-y-2 mb-3">
    <div class="flex items-center gap-2">
        ...
        <button type="button" onclick="mcRemoveOption(this)" class="text-red-400 hover:text-red-600 text-xs px-2 py-1">✕</button>
    </div>
    <div class="flex items-center gap-2">
...
<button type="button" onclick="mcAddOption()"
        class="text-sm text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium">+ Add Option</button>

<script>
function mcAddOption() {
    const list = document.getElementById('mc-inline-options');
    const idx = list.children.length;
    const row = document.createElement('div');
    row.className = 'flex items-center gap-2';
    row.innerHTML = `
        <input type="checkbox" name="correct_options[]" value="${idx}"
               class="h-4 w-4 text-green-600 border-gray-300 dark:border-gray-600">
...
function mcRemoveOption(btn) {
    const list = document.getElementById('mc-inline-options');
    if (list.children.length <= 2) { showToast('Minimum 2 options required.', 'warning'); return; }
    btn.closest('.flex').remove();
    list.querySelectorAll('input[type=checkbox]').forEach((c, i) => c.value = i);
}
</script>
@endif
